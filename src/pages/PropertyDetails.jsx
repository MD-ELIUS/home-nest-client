...
const PropertyDetails = () => {
  ...
  usePageTitle(
    property
      ? `${property.propertyName} | HomeNest Real Estate`
      : "Property Details | HomeNest Real Estate"
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`https://home-nest-api-server-chi.vercel.app/properties/${id}`)
      .then((res) => res.json())
  ...
  useEffect(() => {
    ...
    fetch(`https://home-nest-api-server-chi.vercel.app/reviews?propertyId=${id}`)
      .then((res) => res.json())
  ...
  const handleReviewSubmit = async (e) => {
    ...
    if (!rating || !e.target.reviewText.value.trim()) {
      Swal.fire({
        icon: "warning",
    ...
    if (user?.email === property.userEmail) {
      Swal.fire({
        icon: "error",
        title: "Cannot Review Own Property",
        text: "You cannot review your own property.",
        confirmButtonColor: "#5BA600",
      });
      return;
    }
    ...
    if (userHasReviewed) {
      Swal.fire({
        ...
        text: "You have already submitted a review for this property.",
        confirmButtonColor: "#5BA600",
      });
      return;
    }
    ...
    const newReview = {
      ...
      reviewerImage: user?.photoURL || demoImg,
      propertyId: id,
      propertyName: property?.propertyName,
      rating,
      reviewText: e.target.reviewText.value.trim(),
  ...
  if (!property || loading) return <LoadingData />;
  ...
  return (
    <div className="w-11/12 mx-auto px-4 py-4 sm:py-6 md:py-8 lg:py-10">
      ...
      {/* Info & Posted By */}
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-2 xl:gap-5 animate-fade-in-center mt-6">
        {/* Posted By */}
        <div className="flex flex-col md:justify-between md:items-center md:flex-row gap-2 bg-base-200 rounded-xl p-3 shadow-md border border-base-300">
          <div>
            <p className="font-medium">Posted by</p>
          </div>
          <div className="flex items-center gap-3">
            <img
              src={userImage || demoImg}
              alt="Posted By"
        ...
        {/* Info Grid */}
        <div className="grid grid-cols-1 gap-2 bg-base-200 rounded-xl p-3 shadow-md border border-base-300">
          ...
          <div className="flex flex-col gap-2 md:flex-row justify-between">
            <div className="flex gap-1 items-center">
              ...
              <p className="font-medium text-base flex items-center gap-1">{location}</p>
            </div>
            <div className="flex gap-1 items-center">
      ...
      <div className="mt-10 bg-base-200 border border-base-300 shadow-md rounded-2xl p-4 sm:p-6">
        ...
        {/* Review Form */}
        {user && user?.email !== userEmail && !userHasReviewed ? (
          <form
          ...
          <p className="text-center text-sm md:text-base text-secondary/70 font-medium bg-base-300 py-3 rounded-lg">
            {userHasReviewed
              ...
              : "You cannot review your own property or you are not logged in."}
          </p>
        )}
